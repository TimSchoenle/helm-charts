...
| networkPolicy.cilium | object | `{"description":"","egress":{"dnsMatchPatterns":[],"entityPorts":[],"fqdnPorts":[],"httpRules":[],"toEntities":[],"toFQDNs":[]},"enableDefaultDeny":true,"extraEgress":[],"extraIngress":[],"ingress":{"fromEntities":[]}}` | Cilium-only additions, used when `engine` is `cilium` or `both`. Every rule above is translated automatically; these are the ones the portable API cannot express.  Note that `extraIngress` and `extraEgress` above are *not* carried over: those are verbatim `networking.k8s.io/v1` rule objects and are not valid CNP. The fields below are their counterparts, and the whole tree mirrors the `common` library's so a chart and the library are configured the same way. |
