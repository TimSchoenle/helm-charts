...
single-document charts, and `tankovault` — nine documents, 190 distinct key paths in 591
...
1. **A value that feeds several documents could not say so.** 99 of those 190 paths are declared
...
2. **`unbound` was per document.** Only 53 of the 190 paths are written by any rendered document
   at all; the other 137 are settings the image reads and this chart does not surface. Per
   document that was 387 entries repeating a handful of sentences, so `unbound` moved to the chart
...
The measurement forced all four. What the chart ended up with, as of the measurement: 53 markers
resolving to 219 bindings, and 152 keys written off in six `unbound` entries — eleven the chart
derives from its own topology, sixteen delivered through the secrets directory, and the rest
settings the image reads that this chart offers no value for. The marker and binding counts move
with every key the chart adopts or drops; `just test-contract-scripts` holds the current numbers
in `.github/scripts/tests/snapshots/contract_bindings_gate.json`, regenerated by
`just update-contract-bindings-snapshot` rather than hand-edited here.
