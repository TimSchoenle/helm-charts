...
![Version: 6.0.0](https://img.shields.io/badge/Version-6.0.0-informational?style=flat-square) ![AppVersion: 11.0.1](https://img.shields.io/badge/AppVersion-11.0.1-informational?style=flat-square)
...
The frontend's `api_upstream` deliberately stays plaintext: it is a reverse-proxied public request
rather than an internal call.

#### The probes move to a plaintext port

A kubelet probe presents no client certificate, so on a mutually authenticated listener its plain
`GET /health` is answered with a TLS alert rather than a response — the symptom is
`malformed HTTP response "\x15\x03\x03…"` and every replica killed by its own startup probe. The
probes therefore get a listener of their own on `internal.tls.probePort` (9091), carrying
`/health` and `/ready` and nothing else, and the chart points all three probes at it:

| Service | Probes on | Why |
|---|---|---|
| `control-plane`, `worker`, `sync`, `challenge-solver`, `render` | `probes` (9091) | they serve the mutually authenticated listener |
| `api` | `http` | it holds a certificate to present to peers, but serves its own port to the frontend as ordinary public traffic |
| `notifier` | `http` | it presents its certificate to NATS and serves nothing mutually authenticated |
| `frontend` | `http` | it takes no part in this at all |

The port is opened on the container and **not** published on the Service: the kubelet reaches the
pod directly. The scrape does not move with the probes — it stays on `metrics.port`, so a
deployment that merged the scrape onto an authenticated port keeps it there. Under
`identity: token` none of this applies and the probes answer on the request port as usual.

A probe you have already pointed somewhere yourself is left alone; only a handler still aimed at
the `http` port is moved. If you override one under `mtls`, point it at `probes`.
...
- Some comparisons are derived rather than chosen. `TankoVaultLatencyDegraded` compares p99 against
  ...
  *within* a bucket, so any other value reports an interpolated number rather than an observed
  ...
  quietly disarm paging, so neither is offered. Their traffic `floor` is offered, because how much
  traffic makes a quantile mean anything is a site's own judgement.

### Every ratio has a floor, and household installs are why

A ratio over a counter is undefined below some volume. One failed delivery out of two is 50%, a
p99 over four requests *is* the slowest of those four, and a scan lane that settled three tasks in
fifteen minutes says nothing about whether its provider is broken. Every ratio and quantile alert
this chart ships therefore carries a second conjunct comparing the ratio's own denominator against
a `floor`, recorded as a series in its own right so the alert reads as one thought:

```promql
namespace_channel:notifications_delivered_errors:ratio_rate15m{...} > 0.5
  and namespace_channel:notifications_delivered:rate15m{...} > 0.005
```

The defaults are deliberately low — this chart is installed for one household as often as for
anything larger — and every one of them is a tunable. They are the first thing to raise on an
install that pages on quiet lanes, and the first thing to lower on a large one that wants to hear
about a channel with few subscribers:

```yaml
metrics:
  prometheusRule:
    thresholds:
      # A busy deployment: ignore a lane doing less than a task a minute.
      TankoVaultScanTasksFailing:
        floor: 0.017
      # A single-user install that still wants to know its one email channel is broken.
      TankoVaultNotificationDeliveryFailing:
        floor: 0
```

`rules-tests/volume_floor_test.yml` pins the half that is easy to lose: each of these alerts
staying **quiet** while its ratio sits at 1.0 and its volume sits just under the floor. An alert
that fires on the only three scan tasks of the afternoon is one an operator silences, and after
that the real outage is silent too.
...
Each document under `legal.documents.<slug>` is either served or linked, never both:

| Key | What it is | Delivered as |
|---|---|---|
| `body` | The Markdown text per locale, inline in your values | A ConfigMap, projected into the API pod's secrets directory under `legal__documents__<slug>__body__<locale>` |
| `url` | An absolute `http(s)` link to a document hosted elsewhere | `config.toml`; nothing is mounted |

The chart refuses a document that sets both or neither, naming the slug — the service would
otherwise refuse to boot on the same condition at container start.

The texts stay out of `config.toml`, and so out of the pod's `checksum/config` annotation: the
service reloads its secrets directory when the kubelet refreshes it, so correcting a policy is a
values change and a kubelet sync, never a restart. Adding a document or a locale adds a file,
which the service only watches from its next start.

Chart 6.0.0 follows TankoVault 11, which replaced `legal.dir` and the per-document `sources`
paths with `body`; see [UPGRADING.md](https://github.com/TimSchoenle/helm-charts/blob/main/charts/tankovault/UPGRADING.md#600).
...
TankoVault 7.3.0 fixes the resolver half — it orders resolved addresses IPv6-first, the way a
...
| anilist | object | `{"clientId":"","clientSecret":"","redirectUri":"","tokenEncryptionKey":""}` | The AniList OAuth application accounts are linked through, read by `services.sync`. |
...
| auth | object | `{"jwtSecret":"","mfaEncryptionKey":"","passwordPepper":""}` | The three long-lived secrets the API signs and seals with. Each is generated on a first install and kept across upgrades; the pepper and the MFA key can never change afterwards. |
...
| bootstrap | object | `{"image":{"repository":"timschoenle/tankovault-bootstrap","tag":"v11.0.1@sha256:f2ce8d8965eb72715184d9f17793eefce52b940a352414807152fcd94bc174e0"},"migrate":{"argoSyncWaveBase":0,"backoffLimit":3,"mode":"auto","ordering":"helmHook"},"resources":{"limits":{"memory":"256Mi"},"requests":{"cpu":"50m","memory":"128Mi"}},"seedAdmin":{"email":"","enabled":false,"password":"","username":"admin"},"seedProviders":{"enabled":false}}` | Schema migration and first-install seeding, all from the `bootstrap` image. Nothing published carries a destructive command; resetting the schema is not available in any image. |
| bootstrap.image | object | `{"repository":"timschoenle/tankovault-bootstrap","tag":"v11.0.1@sha256:f2ce8d8965eb72715184d9f17793eefce52b940a352414807152fcd94bc174e0"}` | Repository and tag for the bootstrap image. Registry and pull policy come from the top-level `image`. |
| bootstrap.image.repository | string | `"timschoenle/tankovault-bootstrap"` | Image repository. |
| bootstrap.image.tag | string | `"v11.0.1@sha256:f2ce8d8965eb72715184d9f17793eefce52b940a352414807152fcd94bc174e0"` | Image tag, pinned by digest. |
| bootstrap.migrate | object | `{"argoSyncWaveBase":0,"backoffLimit":3,"mode":"auto","ordering":"helmHook"}` | How and when `bootstrap migrate` runs against the database. |
...
| bootstrap.resources.requests | object | `{"cpu":"50m","memory":"128Mi"}` | Resource requests. |
| bootstrap.resources.requests.cpu | string | `"50m"` | Minimum CPU requested. |
| bootstrap.resources.requests.memory | string | `"128Mi"` | Minimum guaranteed memory allocation. |
| bootstrap.seedAdmin | object | `{"email":"","enabled":false,"password":"","username":"admin"}` | The first administrator, created once on install and never touched again. |
...
| bootstrap.seedProviders | object | `{"enabled":false}` | The built-in provider presets, installed once on install. |
...
| defaults.resources | object | `{"limits":{"memory":"512Mi"},"requests":{"cpu":"100m","memory":"256Mi"}}` | Default resource requests and limits, overridden per service below. A service's own `resources` block is merged over this one, so it only has to state what differs. |
| defaults.resources.limits | object | `{"memory":"512Mi"}` | Resource limits. |
| defaults.resources.limits.memory | string | `"512Mi"` | Maximum allowed memory usage. No CPU limit is set on purpose: a CPU limit does not protect the node the way a memory limit does, it only throttles this container. |
| defaults.resources.requests | object | `{"cpu":"100m","memory":"256Mi"}` | Resource requests. |
| defaults.resources.requests.cpu | string | `"100m"` | Minimum CPU requested. |
| defaults.resources.requests.memory | string | `"256Mi"` | Minimum guaranteed memory allocation. |
...
| defaults.strategy | object | `{}` | Deployment update strategy, used by a service that does not set its own `strategy`. |
...
| image | object | `{"pullPolicy":"","registry":""}` | Registry and pull policy shared by every image this chart runs. The repository and the tag are per component, under `services.<name>.image` and the bundled datastores. |
...
| ingress.tls | object | `{"enabled":false,"secretName":""}` | TLS on the primary Ingress. |
...
| internal | object | `{"identity":"mtls","tls":{"ca":{"bundle":{"create":false,"namespaceSelector":{},"sources":[]},"key":"ca.crt","name":"tankovault-ca","source":"certificateSecret"},"caDir":"/etc/tankovault/tls-ca","certDir":"/etc/tankovault/tls","certManager":{"duration":"2160h","issuerRef":{"group":"cert-manager.io","kind":"ClusterIssuer","name":""},"privateKey":{"algorithm":"ECDSA","rotationPolicy":"Always","size":256},"renewBefore":"360h"},"existingSecrets":{"api":"","challengeSolver":"","controlPlane":"","notifier":"","render":"","sync":"","worker":""},"probePort":9091,"sans":{},"source":"certManager"},"token":"","tokens":{"api":"","worker":""}}` | How the services identify each other on their privileged internal routes. Authorisation is the same either way — every callee carries a compiled-in table of which callers may reach which routes — so this chooses only how a caller proves who it is, and the two modes are otherwise indistinguishable. |
...
| internal.tls.probePort | int | `9091` | Container port carrying `/health` and `/ready` in **plaintext**, beside the mutually authenticated listener. A kubelet probe presents no client certificate, so on the mTLS port its plain `GET /health` is answered with a TLS alert and every replica is killed by its own startup probe — the chart therefore opens this port and points the three probes at it on every service that serves the mTLS listener (`control-plane`, `worker`, `sync`, `challenge-solver`, `render`). `api` and `notifier` hold a certificate but serve no mTLS listener, and their probes are untouched.  It carries the two probes and nothing else: the scrape stays on `metrics.port`, so a deployment that deliberately merged the scrape onto an authenticated port keeps it there. The port is opened on the container only and not published on the Service — the kubelet reaches the pod directly.  There is no off switch: upstream drops the listener only for a literal `null`, which this chart's TOML renderer cannot express, and turning it off would break the probes the chart writes. Ignored entirely under `identity: token`, where the probes answer on the request port like everywhere else. |
...
| legal.documents | string | `nil` | The published documents, keyed by the slug their URL uses. A slug is lowercase letters, digits, `_` and `-`, at most 64 characters, and starts with a letter or a digit (`legal.documents`).  Each document carries exactly one of `body` and `url`, and the chart refuses a document that sets both or neither:  - `body`: the Markdown text per locale. The chart writes it into a ConfigMap and projects each   text into the API pod's secrets directory rather than into `config.toml`, so an edit is   picked up without a restart and does not move `checksum/config`. A new document or locale   is a new file, which the service only watches from its next start. - `url`: an absolute `http(s)` link to a document hosted elsewhere.  `title` is a display name per locale, `updated` a free-form line shown verbatim, and `order` the position in the index, lowest first.  <details><summary>Example</summary>  ```yaml legal:   defaultLocale: en   documents:     terms:       order: 1       updated: "2026-08-04"       title:         en: Terms of Service         de: Nutzungsbedingungen       body:         en: |           # Terms of Service           ...         de: |           # Nutzungsbedingungen           ...     imprint:       title:         de: Impressum       url: https://example.org/impressum ```  </details> |
| metrics | object | `{"dashboard":{"enabled":false,"folder":"","grafanaOperator":{"allowCrossNamespaceImport":true,"enabled":false,"folder":"","instanceSelector":{"matchLabels":{"dashboards":"grafana"}},"resyncPeriod":"5m"},"label":"grafana_dashboard","labelValue":"1","namespace":""},"enabled":true,"natsExporter":{"enabled":false,"image":{"repository":"natsio/prometheus-nats-exporter","tag":"0.20.2@sha256:c623b608e148e31e1c1c878673a197f1828e58ce90de4f01d22f1baa84c8fee9"},"resources":{"limits":{"memory":"64Mi"},"requests":{"cpu":"10m","memory":"32Mi"}},"url":""},"port":9090,"prometheusRule":{"additionalRuleGroups":[],"additionalRuleLabels":{},"disabledAlerts":[],"disabledGroups":[],"enabled":false,"forOverrides":{},"labels":{},"namespace":"","scope":"namespace","severityOverrides":{},"thresholds":{}},"serviceMonitor":{"enabled":false,"interval":"15s","labels":{},"scrapeTimeout":"10s"}}` | Prometheus integration. Every service serves a scrape on an isolated port, outside the request-facing listener and outside its own HTTP metrics middleware. |
| metrics.dashboard | object | `{"enabled":false,"folder":"","grafanaOperator":{"allowCrossNamespaceImport":true,"enabled":false,"folder":"","instanceSelector":{"matchLabels":{"dashboards":"grafana"}},"resyncPeriod":"5m"},"label":"grafana_dashboard","labelValue":"1","namespace":""}` | Grafana dashboard delivery, through a labelled ConfigMap and optionally through grafana-operator. |
| metrics.dashboard.enabled | bool | `false` | Create a ConfigMap holding the upstream Grafana overview dashboard. |
| metrics.dashboard.folder | string | `""` | Grafana folder to file the dashboards under, for both delivery mechanisms: the sidecar reads it from a `grafana_folder` annotation on the ConfigMap and the operator from a field on its own resource. Empty leaves each to its default. |
...
| metrics.dashboard.grafanaOperator.folder | string | `""` | Folder to file the dashboards under, for the operator path only. Overrides `folder` when set; empty falls back to it. |
...
| metrics.natsExporter | object | `{"enabled":false,"image":{"repository":"natsio/prometheus-nats-exporter","tag":"0.20.2@sha256:c623b608e148e31e1c1c878673a197f1828e58ce90de4f01d22f1baa84c8fee9"},"resources":{"limits":{"memory":"64Mi"},"requests":{"cpu":"10m","memory":"32Mi"}},"url":""}` | The sidecar that turns NATS' own monitoring protocol into a Prometheus exposition. |
...
| metrics.natsExporter.image | object | `{"repository":"natsio/prometheus-nats-exporter","tag":"0.20.2@sha256:c623b608e148e31e1c1c878673a197f1828e58ce90de4f01d22f1baa84c8fee9"}` | Repository and tag for the exporter image. |
| metrics.natsExporter.image.repository | string | `"natsio/prometheus-nats-exporter"` | Image repository. |
| metrics.natsExporter.image.tag | string | `"0.20.2@sha256:c623b608e148e31e1c1c878673a197f1828e58ce90de4f01d22f1baa84c8fee9"` | Image tag, pinned by digest. |
| metrics.natsExporter.resources | object | `{"limits":{"memory":"64Mi"},"requests":{"cpu":"10m","memory":"32Mi"}}` | Resource requests and limits for the exporter. It polls two JSON endpoints and re-serves them; it does no work proportional to traffic. |
| metrics.natsExporter.resources.limits | object | `{"memory":"64Mi"}` | Resource limits. |
| metrics.natsExporter.resources.limits.memory | string | `"64Mi"` | Maximum allowed memory usage. |
| metrics.natsExporter.resources.requests | object | `{"cpu":"10m","memory":"32Mi"}` | Resource requests. |
| metrics.natsExporter.resources.requests.cpu | string | `"10m"` | Minimum CPU requested. |
...
| metrics.prometheusRule | object | `{"additionalRuleGroups":[],"additionalRuleLabels":{},"disabledAlerts":[],"disabledGroups":[],"enabled":false,"forOverrides":{},"labels":{},"namespace":"","scope":"namespace","severityOverrides":{},"thresholds":{}}` | The recording and alerting rules under `rules/`, installed as a PrometheusRule. |
...
| nats.resources | object | `{"limits":{"memory":"512Mi"},"requests":{"cpu":"100m","memory":"256Mi"}}` | Resource requests and limits for the NATS container. JetStream keeps its working set in memory, so raise the limit alongside the stream retention. |
| nats.resources.limits | object | `{"memory":"512Mi"}` | Resource limits. |
| nats.resources.limits.memory | string | `"512Mi"` | Maximum allowed memory usage. |
...
| networkPolicy.ingressController | object | `{"namespaceSelector":{},"podSelector":{}}` | Selectors matching the ingress controller. Read only while an Ingress is enabled, and only for the service that Ingress publishes. |
...
| postgresql.image | object | `{"repository":"pgvector/pgvector","tag":"pg18@sha256:2ba9ca5f2e7daa0f0e7723cba1ee9167bab54efd3640516a44ac1a928dd67e7a"}` | Repository and tag for the bundled PostgreSQL. |
| postgresql.image.repository | string | `"pgvector/pgvector"` | Image repository. `pgvector/pgvector`, not stock `postgres`: TankoVault migration `0027_recsys_signals` runs `CREATE EXTENSION vector` and the recommender retrieves through an HNSW index, so the extension is a hard dependency of the deployment rather than an optional extra. This is the official PostgreSQL image with pgvector preinstalled — same major, same entrypoint, same environment contract, same `postgres` uid — so nothing else in this block changes and an existing PVC is reused in place. It is Debian-based rather than Alpine, which `postgresql.resources` below already accommodates. |
| postgresql.image.tag | string | `"pg18@sha256:2ba9ca5f2e7daa0f0e7723cba1ee9167bab54efd3640516a44ac1a928dd67e7a"` | Image tag, pinned by digest. Held to the same digest as upstream's compose stack and CI services, so the database this chart runs is the one the queries were type-checked against. |
| postgresql.persistence | object | `{"enabled":true,"existingClaim":"","size":"20Gi","storageClassName":""}` | The volume the data directory lives on. |
...
| postgresql.resources | object | `{"limits":{"memory":"1Gi"},"requests":{"cpu":"250m","memory":"512Mi"}}` | Resource requests and limits for the database container. This is the evaluation-tier datastore; a production PostgreSQL is sized by whoever runs it. |
| postgresql.resources.limits | object | `{"memory":"1Gi"}` | Resource limits. |
| postgresql.resources.limits.memory | string | `"1Gi"` | Maximum allowed memory usage. No CPU limit is set on purpose: a CPU limit does not protect the node the way a memory limit does, it only throttles this container. |
| postgresql.resources.requests | object | `{"cpu":"250m","memory":"512Mi"}` | Resource requests. |
| postgresql.resources.requests.cpu | string | `"250m"` | Minimum CPU requested. |
| postgresql.resources.requests.memory | string | `"512Mi"` | Minimum guaranteed memory allocation. |
...
| scheduler | object | `{"chapterRollupVerifyIntervalSecs":1200,"scanHistoryPruneIntervalSecs":7200,"scanHistoryRetentionDays":14,"seriesBrowseVerifyIntervalSecs":1800,"unreadReconcileIntervalSecs":3600,"unreadUnlockIntervalSecs":300,"watchlistImportResolveIntervalSecs":3600}` | The `scheduler` settings. |
| scheduler.chapterRollupVerifyIntervalSecs | int | `1200` | Seconds between passes that re-count a batch of sources' chapters and rebuild any whose stored console totals disagree. 0 disables (`scheduler.chapter_rollup_verify_interval_secs`). |
| scheduler.scanHistoryPruneIntervalSecs | int | `7200` | Seconds between scan-history pruning passes. 0 disables (`scheduler.scan_history_prune_interval_secs`). |
| scheduler.scanHistoryRetentionDays | int | `14` | Days settled scan runs and their tasks are kept. 0 keeps them forever (`scheduler.scan_history_retention_days`). |
| scheduler.seriesBrowseVerifyIntervalSecs | int | `1800` | Seconds between passes that recompute a batch of series' browse keys and repair any whose stored projection disagrees. 0 disables (`scheduler.series_browse_verify_interval_secs`). |
| scheduler.unreadReconcileIntervalSecs | int | `3600` | Seconds between passes that re-verify stored unread figures against the live computation and repair any that drifted. 0 disables (`scheduler.unread_reconcile_interval_secs`). |
| scheduler.unreadUnlockIntervalSecs | int | `300` | Seconds between passes that recompute stored unread figures whose early-access unlock time has passed. 0 disables (`scheduler.unread_unlock_interval_secs`). |
| scheduler.watchlistImportResolveIntervalSecs | int | `3600` | Seconds between passes that retry imported watchlist entries still waiting for their series, attaching each one a scan has since brought into the catalogue. 0 disables (`scheduler.watchlist_import_resolve_interval_secs`). |
| serviceAccount | object | `{"annotations":{},"create":true,"name":""}` | ServiceAccount every workload runs under. `create: false` with no `name` means the `default` one. |
...
| services | object | `{"api":{"affinity":{},"autoscaling":{"enabled":false,"maxReplicas":10,"minReplicas":2,"targetCPUUtilizationPercentage":80,"targetMemoryUtilizationPercentage":null},"config":{},"enabled":true,"image":{"repository":"timschoenle/tankovault-api","tag":"v11.0.1@sha256:f3ef3dd4f4470319755c364fa5451fd47008fef6069d9d6c82d0a91589823a09"},"nodeSelector":{},"podAntiAffinity":"","podDisruptionBudget":{"enabled":false,"maxUnavailable":null,"minAvailable":1},"priorityClassName":"","replicaCount":2,"resources":{"limits":{"memory":"1Gi"},"requests":{"cpu":"250m","memory":"512Mi"}},"service":{"annotations":{},"type":"ClusterIP"},"strategy":{},"tolerations":[],"topologySpreadConstraints":[]},"challengeSolver":{"affinity":{},"autoscaling":{"enabled":false,"maxReplicas":4,"minReplicas":1,"targetCPUUtilizationPercentage":80,"targetMemoryUtilizationPercentage":null},"config":{},"enabled":true,"image":{"repository":"timschoenle/tankovault-challenge-solver","tag":"v11.0.1@sha256:a078767b821a16dc71f8be322b8c3cc66d35707f04bc94d6157406137c0dbf93"},"nodeSelector":{},"podAntiAffinity":"","podDisruptionBudget":{"enabled":false,"maxUnavailable":null,"minAvailable":1},"priorityClassName":"","replicaCount":1,"resources":{"limits":{"memory":"512Mi"},"requests":{"cpu":"100m","memory":"256Mi"}},"service":{"annotations":{},"type":"ClusterIP"},"strategy":{},"tolerations":[],"topologySpreadConstraints":[]},"controlPlane":{"affinity":{},"config":{},"enabled":true,"image":{"repository":"timschoenle/tankovault-control-plane","tag":"v11.0.1@sha256:b2a9f22edd96a7afa5d62e432525bb48a39afdb9b6beda553d2ebab83ab4431c"},"nodeSelector":{},"podAntiAffinity":"","podDisruptionBudget":{"enabled":false,"maxUnavailable":null,"minAvailable":1},"priorityClassName":"","replicaCount":1,"resources":{"limits":{"memory":"512Mi"},"requests":{"cpu":"100m","memory":"256Mi"}},"service":{"annotations":{},"type":"ClusterIP"},"strategy":{},"tolerations":[],"topologySpreadConstraints":[]},"frontend":{"affinity":{},"autoscaling":{"enabled":false,"maxReplicas":6,"minReplicas":2,"targetCPUUtilizationPercentage":80,"targetMemoryUtilizationPercentage":null},"config":{},"enabled":true,"image":{"repository":"timschoenle/tankovault-frontend","tag":"v11.0.1@sha256:49119a5ea0c532ef4655b41d67d77ccf5316f06a66b4dfa8e59c7f10bf23c6d3"},"nodeSelector":{},"podAntiAffinity":"","podDisruptionBudget":{"enabled":false,"maxUnavailable":null,"minAvailable":1},"priorityClassName":"","replicaCount":2,"resources":{"limits":{"memory":"256Mi"},"requests":{"cpu":"50m","memory":"128Mi"}},"service":{"annotations":{},"type":"ClusterIP"},"strategy":{},"tolerations":[],"topologySpreadConstraints":[]},"notifier":{"affinity":{},"config":{},"enabled":true,"image":{"repository":"timschoenle/tankovault-notifier","tag":"v11.0.1@sha256:d7027f960fb6451f3f1fd33304b149b20d338d38d8abc13cbf02797c4d0a395f"},"nodeSelector":{},"podAntiAffinity":"","podDisruptionBudget":{"enabled":false,"maxUnavailable":null,"minAvailable":1},"priorityClassName":"","replicaCount":1,"resources":{"limits":{"memory":"512Mi"},"requests":{"cpu":"100m","memory":"256Mi"}},"service":{"annotations":{},"type":"ClusterIP"},"strategy":{},"tolerations":[],"topologySpreadConstraints":[]},"render":{"affinity":{},"autoscaling":{"enabled":false,"maxReplicas":4,"minReplicas":1,"targetCPUUtilizationPercentage":80,"targetMemoryUtilizationPercentage":null},"config":{},"enabled":false,"homeDir":"/home/nonroot","image":{"repository":"timschoenle/tankovault-render","tag":"v11.0.1@sha256:fda7f7f3a71335f292cb656714f1ab92626230d496f5638db18dc308c89a38d6"},"nodeSelector":{},"podAntiAffinity":"","podDisruptionBudget":{"enabled":false,"maxUnavailable":null,"minAvailable":1},"priorityClassName":"","replicaCount":1,"resources":{"limits":{"memory":"2Gi"},"requests":{"cpu":"250m","memory":"512Mi"}},"service":{"annotations":{},"type":"ClusterIP"},"shmSize":"1Gi","strategy":{},"tolerations":[],"topologySpreadConstraints":[]},"sync":{"affinity":{},"config":{},"enabled":true,"image":{"repository":"timschoenle/tankovault-sync","tag":"v11.0.1@sha256:f6e79954546976ad40d877355731537626bd4fe3227a964b1a54c87b1b822e29"},"nodeSelector":{},"podAntiAffinity":"","podDisruptionBudget":{"enabled":false,"maxUnavailable":null,"minAvailable":1},"priorityClassName":"","replicaCount":1,"resources":{"limits":{"memory":"512Mi"},"requests":{"cpu":"100m","memory":"256Mi"}},"service":{"annotations":{},"type":"ClusterIP"},"strategy":{},"tolerations":[],"topologySpreadConstraints":[]},"worker":{"affinity":{},"autoscaling":{"enabled":false,"maxReplicas":10,"minReplicas":2,"targetCPUUtilizationPercentage":80,"targetMemoryUtilizationPercentage":null},"config":{},"enabled":true,"image":{"repository":"timschoenle/tankovault-worker","tag":"v11.0.1@sha256:11e0ae2b72415625d5577e044a6e9147d2017bd2fedec5d731c2b454301037f9"},"nodeSelector":{},"podAntiAffinity":"","podDisruptionBudget":{"enabled":false,"maxUnavailable":null,"minAvailable":1},"priorityClassName":"","replicaCount":2,"resources":{"limits":{"memory":"1Gi"},"requests":{"cpu":"250m","memory":"512Mi"}},"service":{"annotations":{},"type":"ClusterIP"},"strategy":{},"tolerations":[],"topologySpreadConstraints":[]}}` | Per-service settings. Each block is merged over `defaults`, so any key from `defaults` may be repeated here for one service only. |
| services.api | object | `{"affinity":{},"autoscaling":{"enabled":false,"maxReplicas":10,"minReplicas":2,"targetCPUUtilizationPercentage":80,"targetMemoryUtilizationPercentage":null},"config":{},"enabled":true,"image":{"repository":"timschoenle/tankovault-api","tag":"v11.0.1@sha256:f3ef3dd4f4470319755c364fa5451fd47008fef6069d9d6c82d0a91589823a09"},"nodeSelector":{},"podAntiAffinity":"","podDisruptionBudget":{"enabled":false,"maxUnavailable":null,"minAvailable":1},"priorityClassName":"","replicaCount":2,"resources":{"limits":{"memory":"1Gi"},"requests":{"cpu":"250m","memory":"512Mi"}},"service":{"annotations":{},"type":"ClusterIP"},"strategy":{},"tolerations":[],"topologySpreadConstraints":[]}` | The axum REST edge: authentication, read models, write endpoints, administration and the server-sent scan feed. |
| services.api.affinity | object | `{}` | Explicit affinity rules for this service, overriding `defaults.affinity`. Wins over `podAntiAffinity` when set. |
| services.api.autoscaling | object | `{"enabled":false,"maxReplicas":10,"minReplicas":2,"targetCPUUtilizationPercentage":80,"targetMemoryUtilizationPercentage":null}` | HorizontalPodAutoscaler for this service. While it is enabled the Deployment renders no `replicas`, so `replicaCount` is ignored. |
...
| services.api.image | object | `{"repository":"timschoenle/tankovault-api","tag":"v11.0.1@sha256:f3ef3dd4f4470319755c364fa5451fd47008fef6069d9d6c82d0a91589823a09"}` | Repository and tag for this service. Registry and pull policy come from the top-level `image`. |
| services.api.image.repository | string | `"timschoenle/tankovault-api"` | Image repository. |
| services.api.image.tag | string | `"v11.0.1@sha256:f3ef3dd4f4470319755c364fa5451fd47008fef6069d9d6c82d0a91589823a09"` | Image tag, pinned by digest. |
| services.api.nodeSelector | object | `{}` | Node selector for this service's pods, overriding `defaults.nodeSelector`. |
| services.api.podAntiAffinity | string | `""` | Pod anti-affinity across nodes for this service, overriding `defaults.podAntiAffinity`. Empty defers to the default. `soft` prefers spreading replicas, `hard` requires it. |
| services.api.podDisruptionBudget | object | `{"enabled":false,"maxUnavailable":null,"minAvailable":1}` | PodDisruptionBudget for this service's pods. |
...
| services.api.priorityClassName | string | `""` | Priority class for this service's pods, overriding `defaults.priorityClassName`. |
| services.api.replicaCount | int | `2` | Replica count, ignored when autoscaling is enabled. |
| services.api.resources | object | `{"limits":{"memory":"1Gi"},"requests":{"cpu":"250m","memory":"512Mi"}}` | Resource requests and limits. The API terminates every request and holds the recommender's working set, so it is the largest of the services. |
| services.api.resources.limits | object | `{"memory":"1Gi"}` | Resource limits. |
| services.api.resources.limits.memory | string | `"1Gi"` | Maximum allowed memory usage. |
| services.api.resources.requests | object | `{"cpu":"250m","memory":"512Mi"}` | Resource requests. |
| services.api.resources.requests.cpu | string | `"250m"` | Minimum CPU requested. |
| services.api.resources.requests.memory | string | `"512Mi"` | Minimum guaranteed memory allocation. |
| services.api.service | object | `{"annotations":{},"type":"ClusterIP"}` | The Service in front of this deployment. Its port is fixed by the chart, so only the type and the annotations are open. |
| services.api.service.annotations | object | `{}` | Extra Service annotations. |
| services.api.service.type | string | `"ClusterIP"` | Service type. |
| services.api.strategy | object | `{}` | Deployment update strategy for this service, overriding `defaults.strategy`. |
| services.api.tolerations | list | `[]` | Tolerations for this service's pods, overriding `defaults.tolerations`. |
| services.api.topologySpreadConstraints | list | `[]` | Topology spread constraints for this service's pods, overriding `defaults.topologySpreadConstraints`. |
| services.challengeSolver | object | `{"affinity":{},"autoscaling":{"enabled":false,"maxReplicas":4,"minReplicas":1,"targetCPUUtilizationPercentage":80,"targetMemoryUtilizationPercentage":null},"config":{},"enabled":true,"image":{"repository":"timschoenle/tankovault-challenge-solver","tag":"v11.0.1@sha256:a078767b821a16dc71f8be322b8c3cc66d35707f04bc94d6157406137c0dbf93"},"nodeSelector":{},"podAntiAffinity":"","podDisruptionBudget":{"enabled":false,"maxUnavailable":null,"minAvailable":1},"priorityClassName":"","replicaCount":1,"resources":{"limits":{"memory":"512Mi"},"requests":{"cpu":"100m","memory":"256Mi"}},"service":{"annotations":{},"type":"ClusterIP"},"strategy":{},"tolerations":[],"topologySpreadConstraints":[]}` | Modular bot-management bypass tier. Detects Cloudflare, JavaScript and Turnstile interstitials and delegates to a solver backend, TRAWL by default. |
| services.challengeSolver.affinity | object | `{}` | Explicit affinity rules for this service, overriding `defaults.affinity`. Wins over `podAntiAffinity` when set. |
| services.challengeSolver.autoscaling | object | `{"enabled":false,"maxReplicas":4,"minReplicas":1,"targetCPUUtilizationPercentage":80,"targetMemoryUtilizationPercentage":null}` | HorizontalPodAutoscaler for this service. While it is enabled the Deployment renders no `replicas`, so `replicaCount` is ignored. |
...
| services.challengeSolver.image | object | `{"repository":"timschoenle/tankovault-challenge-solver","tag":"v11.0.1@sha256:a078767b821a16dc71f8be322b8c3cc66d35707f04bc94d6157406137c0dbf93"}` | Repository and tag for this service. Registry and pull policy come from the top-level `image`. |
| services.challengeSolver.image.repository | string | `"timschoenle/tankovault-challenge-solver"` | Image repository. |
| services.challengeSolver.image.tag | string | `"v11.0.1@sha256:a078767b821a16dc71f8be322b8c3cc66d35707f04bc94d6157406137c0dbf93"` | Image tag, pinned by digest. |
| services.challengeSolver.nodeSelector | object | `{}` | Node selector for this service's pods, overriding `defaults.nodeSelector`. |
| services.challengeSolver.podAntiAffinity | string | `""` | Pod anti-affinity across nodes for this service, overriding `defaults.podAntiAffinity`. Empty defers to the default. `soft` prefers spreading replicas, `hard` requires it. |
| services.challengeSolver.podDisruptionBudget | object | `{"enabled":false,"maxUnavailable":null,"minAvailable":1}` | PodDisruptionBudget for this service's pods. |
...
| services.challengeSolver.priorityClassName | string | `""` | Priority class for this service's pods, overriding `defaults.priorityClassName`. |
| services.challengeSolver.replicaCount | int | `1` | Replica count, ignored when autoscaling is enabled. |
| services.challengeSolver.resources | object | `{"limits":{"memory":"512Mi"},"requests":{"cpu":"100m","memory":"256Mi"}}` | Resource requests and limits. The solver brokers work to the render tier or to TRAWL; it runs no browser itself. |
| services.challengeSolver.resources.limits | object | `{"memory":"512Mi"}` | Resource limits. |
| services.challengeSolver.resources.limits.memory | string | `"512Mi"` | Maximum allowed memory usage. |
| services.challengeSolver.resources.requests | object | `{"cpu":"100m","memory":"256Mi"}` | Resource requests. |
| services.challengeSolver.resources.requests.cpu | string | `"100m"` | Minimum CPU requested. |
| services.challengeSolver.resources.requests.memory | string | `"256Mi"` | Minimum guaranteed memory allocation. |
| services.challengeSolver.service | object | `{"annotations":{},"type":"ClusterIP"}` | The Service in front of this deployment. Its port is fixed by the chart, so only the type and the annotations are open. |
...
| services.challengeSolver.strategy | object | `{}` | Deployment update strategy for this service, overriding `defaults.strategy`. |
| services.challengeSolver.tolerations | list | `[]` | Tolerations for this service's pods, overriding `defaults.tolerations`. |
| services.challengeSolver.topologySpreadConstraints | list | `[]` | Topology spread constraints for this service's pods, overriding `defaults.topologySpreadConstraints`. |
| services.controlPlane | object | `{"affinity":{},"config":{},"enabled":true,"image":{"repository":"timschoenle/tankovault-control-plane","tag":"v11.0.1@sha256:b2a9f22edd96a7afa5d62e432525bb48a39afdb9b6beda553d2ebab83ab4431c"},"nodeSelector":{},"podAntiAffinity":"","podDisruptionBudget":{"enabled":false,"maxUnavailable":null,"minAvailable":1},"priorityClassName":"","replicaCount":1,"resources":{"limits":{"memory":"512Mi"},"requests":{"cpu":"100m","memory":"256Mi"}},"service":{"annotations":{},"type":"ClusterIP"},"strategy":{},"tolerations":[],"topologySpreadConstraints":[]}` | The singleton scheduler: run planning, task distribution and provider health. Safe to run with more than one replica — it elects a leader through Redis, and falls open to sole-leader when Redis is absent. |
| services.controlPlane.affinity | object | `{}` | Explicit affinity rules for this service, overriding `defaults.affinity`. Wins over `podAntiAffinity` when set. |
...
| services.controlPlane.image | object | `{"repository":"timschoenle/tankovault-control-plane","tag":"v11.0.1@sha256:b2a9f22edd96a7afa5d62e432525bb48a39afdb9b6beda553d2ebab83ab4431c"}` | Repository and tag for this service. Registry and pull policy come from the top-level `image`. |
| services.controlPlane.image.repository | string | `"timschoenle/tankovault-control-plane"` | Image repository. |
| services.controlPlane.image.tag | string | `"v11.0.1@sha256:b2a9f22edd96a7afa5d62e432525bb48a39afdb9b6beda553d2ebab83ab4431c"` | Image tag, pinned by digest. |
| services.controlPlane.nodeSelector | object | `{}` | Node selector for this service's pods, overriding `defaults.nodeSelector`. |
| services.controlPlane.podAntiAffinity | string | `""` | Pod anti-affinity across nodes for this service, overriding `defaults.podAntiAffinity`. Empty defers to the default. `soft` prefers spreading replicas, `hard` requires it. |
| services.controlPlane.podDisruptionBudget | object | `{"enabled":false,"maxUnavailable":null,"minAvailable":1}` | PodDisruptionBudget for this service's pods. |
...
| services.controlPlane.priorityClassName | string | `""` | Priority class for this service's pods, overriding `defaults.priorityClassName`. |
| services.controlPlane.replicaCount | int | `1` | Replica count. |
| services.controlPlane.resources | object | `{"limits":{"memory":"512Mi"},"requests":{"cpu":"100m","memory":"256Mi"}}` | Resource requests and limits. The control plane schedules work and holds no per-request state. |
| services.controlPlane.resources.limits | object | `{"memory":"512Mi"}` | Resource limits. |
| services.controlPlane.resources.limits.memory | string | `"512Mi"` | Maximum allowed memory usage. |
| services.controlPlane.resources.requests | object | `{"cpu":"100m","memory":"256Mi"}` | Resource requests. |
| services.controlPlane.resources.requests.cpu | string | `"100m"` | Minimum CPU requested. |
| services.controlPlane.resources.requests.memory | string | `"256Mi"` | Minimum guaranteed memory allocation. |
| services.controlPlane.service | object | `{"annotations":{},"type":"ClusterIP"}` | The Service in front of this deployment. Its port is fixed by the chart, so only the type and the annotations are open. |
...
| services.controlPlane.strategy | object | `{}` | Deployment update strategy for this service, overriding `defaults.strategy`. |
| services.controlPlane.tolerations | list | `[]` | Tolerations for this service's pods, overriding `defaults.tolerations`. |
| services.controlPlane.topologySpreadConstraints | list | `[]` | Topology spread constraints for this service's pods, overriding `defaults.topologySpreadConstraints`. |
| services.frontend | object | `{"affinity":{},"autoscaling":{"enabled":false,"maxReplicas":6,"minReplicas":2,"targetCPUUtilizationPercentage":80,"targetMemoryUtilizationPercentage":null},"config":{},"enabled":true,"image":{"repository":"timschoenle/tankovault-frontend","tag":"v11.0.1@sha256:49119a5ea0c532ef4655b41d67d77ccf5316f06a66b4dfa8e59c7f10bf23c6d3"},"nodeSelector":{},"podAntiAffinity":"","podDisruptionBudget":{"enabled":false,"maxUnavailable":null,"minAvailable":1},"priorityClassName":"","replicaCount":2,"resources":{"limits":{"memory":"256Mi"},"requests":{"cpu":"50m","memory":"128Mi"}},"service":{"annotations":{},"type":"ClusterIP"},"strategy":{},"tolerations":[],"topologySpreadConstraints":[]}` | The Dioxus WASM SPA and its axum server. It serves the client and reverse-proxies `/v1/*` to the API, so this single origin is all a browser needs — which is why it is the only service the ingress exposes. |
| services.frontend.affinity | object | `{}` | Explicit affinity rules for this service, overriding `defaults.affinity`. Wins over `podAntiAffinity` when set. |
| services.frontend.autoscaling | object | `{"enabled":false,"maxReplicas":6,"minReplicas":2,"targetCPUUtilizationPercentage":80,"targetMemoryUtilizationPercentage":null}` | HorizontalPodAutoscaler for this service. While it is enabled the Deployment renders no `replicas`, so `replicaCount` is ignored. |
...
| services.frontend.image | object | `{"repository":"timschoenle/tankovault-frontend","tag":"v11.0.1@sha256:49119a5ea0c532ef4655b41d67d77ccf5316f06a66b4dfa8e59c7f10bf23c6d3"}` | Repository and tag for this service. Registry and pull policy come from the top-level `image`. |
| services.frontend.image.repository | string | `"timschoenle/tankovault-frontend"` | Image repository. |
| services.frontend.image.tag | string | `"v11.0.1@sha256:49119a5ea0c532ef4655b41d67d77ccf5316f06a66b4dfa8e59c7f10bf23c6d3"` | Image tag, pinned by digest. |
| services.frontend.nodeSelector | object | `{}` | Node selector for this service's pods, overriding `defaults.nodeSelector`. |
| services.frontend.podAntiAffinity | string | `""` | Pod anti-affinity across nodes for this service, overriding `defaults.podAntiAffinity`. Empty defers to the default. `soft` prefers spreading replicas, `hard` requires it. |
| services.frontend.podDisruptionBudget | object | `{"enabled":false,"maxUnavailable":null,"minAvailable":1}` | PodDisruptionBudget for this service's pods. |
...
| services.frontend.priorityClassName | string | `""` | Priority class for this service's pods, overriding `defaults.priorityClassName`. |
| services.frontend.replicaCount | int | `2` | Replica count, ignored when autoscaling is enabled. |
| services.frontend.resources | object | `{"limits":{"memory":"256Mi"},"requests":{"cpu":"50m","memory":"128Mi"}}` | Resource requests and limits. The frontend serves static assets and proxies `/v1/*`; it does no work proportional to library size. |
| services.frontend.resources.limits | object | `{"memory":"256Mi"}` | Resource limits. |
| services.frontend.resources.limits.memory | string | `"256Mi"` | Maximum allowed memory usage. |
| services.frontend.resources.requests | object | `{"cpu":"50m","memory":"128Mi"}` | Resource requests. |
| services.frontend.resources.requests.cpu | string | `"50m"` | Minimum CPU requested. |
| services.frontend.resources.requests.memory | string | `"128Mi"` | Minimum guaranteed memory allocation. |
| services.frontend.service | object | `{"annotations":{},"type":"ClusterIP"}` | The Service in front of this deployment. Its port is fixed by the chart, so only the type and the annotations are open. |
...
| services.frontend.strategy | object | `{}` | Deployment update strategy for this service, overriding `defaults.strategy`. |
| services.frontend.tolerations | list | `[]` | Tolerations for this service's pods, overriding `defaults.tolerations`. |
| services.frontend.topologySpreadConstraints | list | `[]` | Topology spread constraints for this service's pods, overriding `defaults.topologySpreadConstraints`. |
| services.notifier | object | `{"affinity":{},"config":{},"enabled":true,"image":{"repository":"timschoenle/tankovault-notifier","tag":"v11.0.1@sha256:d7027f960fb6451f3f1fd33304b149b20d338d38d8abc13cbf02797c4d0a395f"},"nodeSelector":{},"podAntiAffinity":"","podDisruptionBudget":{"enabled":false,"maxUnavailable":null,"minAvailable":1},"priorityClassName":"","replicaCount":1,"resources":{"limits":{"memory":"512Mi"},"requests":{"cpu":"100m","memory":"256Mi"}},"service":{"annotations":{},"type":"ClusterIP"},"strategy":{},"tolerations":[],"topologySpreadConstraints":[]}` | Distributes new-chapter notifications to users over email, Discord and generic webhooks. |
| services.notifier.affinity | object | `{}` | Explicit affinity rules for this service, overriding `defaults.affinity`. Wins over `podAntiAffinity` when set. |
...
| services.notifier.image | object | `{"repository":"timschoenle/tankovault-notifier","tag":"v11.0.1@sha256:d7027f960fb6451f3f1fd33304b149b20d338d38d8abc13cbf02797c4d0a395f"}` | Repository and tag for this service. Registry and pull policy come from the top-level `image`. |
| services.notifier.image.repository | string | `"timschoenle/tankovault-notifier"` | Image repository. |
| services.notifier.image.tag | string | `"v11.0.1@sha256:d7027f960fb6451f3f1fd33304b149b20d338d38d8abc13cbf02797c4d0a395f"` | Image tag, pinned by digest. |
| services.notifier.nodeSelector | object | `{}` | Node selector for this service's pods, overriding `defaults.nodeSelector`. |
| services.notifier.podAntiAffinity | string | `""` | Pod anti-affinity across nodes for this service, overriding `defaults.podAntiAffinity`. Empty defers to the default. `soft` prefers spreading replicas, `hard` requires it. |
| services.notifier.podDisruptionBudget | object | `{"enabled":false,"maxUnavailable":null,"minAvailable":1}` | PodDisruptionBudget for this service's pods. |
...
| services.notifier.priorityClassName | string | `""` | Priority class for this service's pods, overriding `defaults.priorityClassName`. |
| services.notifier.replicaCount | int | `1` | Replica count. One by default; upstream runs this tier as a singleton. |
| services.notifier.resources | object | `{"limits":{"memory":"512Mi"},"requests":{"cpu":"100m","memory":"256Mi"}}` | Resource requests and limits. The notifier renders templates and speaks SMTP and HTTP; its footprint is bounded by the outbound queue. |
| services.notifier.resources.limits | object | `{"memory":"512Mi"}` | Resource limits. |
| services.notifier.resources.limits.memory | string | `"512Mi"` | Maximum allowed memory usage. |
| services.notifier.resources.requests | object | `{"cpu":"100m","memory":"256Mi"}` | Resource requests. |
| services.notifier.resources.requests.cpu | string | `"100m"` | Minimum CPU requested. |
| services.notifier.resources.requests.memory | string | `"256Mi"` | Minimum guaranteed memory allocation. |
| services.notifier.service | object | `{"annotations":{},"type":"ClusterIP"}` | The Service in front of this deployment. Its port is fixed by the chart, so only the type and the annotations are open. |
...
| services.notifier.strategy | object | `{}` | Deployment update strategy for this service, overriding `defaults.strategy`. |
| services.notifier.tolerations | list | `[]` | Tolerations for this service's pods, overriding `defaults.tolerations`. |
| services.notifier.topologySpreadConstraints | list | `[]` | Topology spread constraints for this service's pods, overriding `defaults.topologySpreadConstraints`. |
| services.render | object | `{"affinity":{},"autoscaling":{"enabled":false,"maxReplicas":4,"minReplicas":1,"targetCPUUtilizationPercentage":80,"targetMemoryUtilizationPercentage":null},"config":{},"enabled":false,"homeDir":"/home/nonroot","image":{"repository":"timschoenle/tankovault-render","tag":"v11.0.1@sha256:fda7f7f3a71335f292cb656714f1ab92626230d496f5638db18dc308c89a38d6"},"nodeSelector":{},"podAntiAffinity":"","podDisruptionBudget":{"enabled":false,"maxUnavailable":null,"minAvailable":1},"priorityClassName":"","replicaCount":1,"resources":{"limits":{"memory":"2Gi"},"requests":{"cpu":"250m","memory":"512Mi"}},"service":{"annotations":{},"type":"ClusterIP"},"shmSize":"1Gi","strategy":{},"tolerations":[],"topologySpreadConstraints":[]}` | Optional headless-browser tier for JavaScript-rendered pages; doubles as a solver backend. This is the one service not built on `scratch`: it is a Debian base driving a real Chromium, so it needs writable scratch space and a shared-memory volume. |
| services.render.affinity | object | `{}` | Explicit affinity rules for this service, overriding `defaults.affinity`. Wins over `podAntiAffinity` when set. |
| services.render.autoscaling | object | `{"enabled":false,"maxReplicas":4,"minReplicas":1,"targetCPUUtilizationPercentage":80,"targetMemoryUtilizationPercentage":null}` | HorizontalPodAutoscaler for this service. While it is enabled the Deployment renders no `replicas`, so `replicaCount` is ignored. |
...
| services.render.image | object | `{"repository":"timschoenle/tankovault-render","tag":"v11.0.1@sha256:fda7f7f3a71335f292cb656714f1ab92626230d496f5638db18dc308c89a38d6"}` | Repository and tag for this service. Registry and pull policy come from the top-level `image`. |
| services.render.image.repository | string | `"timschoenle/tankovault-render"` | Image repository. |
| services.render.image.tag | string | `"v11.0.1@sha256:fda7f7f3a71335f292cb656714f1ab92626230d496f5638db18dc308c89a38d6"` | Image tag, pinned by digest. |
| services.render.nodeSelector | object | `{}` | Node selector for this service's pods, overriding `defaults.nodeSelector`. |
| services.render.podAntiAffinity | string | `""` | Pod anti-affinity across nodes for this service, overriding `defaults.podAntiAffinity`. Empty defers to the default. `soft` prefers spreading replicas, `hard` requires it. |
| services.render.podDisruptionBudget | object | `{"enabled":false,"maxUnavailable":null,"minAvailable":1}` | PodDisruptionBudget for this service's pods. |
...
| services.render.priorityClassName | string | `""` | Priority class for this service's pods, overriding `defaults.priorityClassName`. |
| services.render.replicaCount | int | `1` | Replica count, ignored when autoscaling is enabled. |
| services.render.resources | object | `{"limits":{"memory":"2Gi"},"requests":{"cpu":"250m","memory":"512Mi"}}` | Resource requests and limits. Chromium needs far more headroom than the rest of the stack, and the memory limit is what keeps a runaway renderer off the node. |
...
| services.render.service | object | `{"annotations":{},"type":"ClusterIP"}` | The Service in front of this deployment. Its port is fixed by the chart, so only the type and the annotations are open. |
...
| services.render.strategy | object | `{}` | Deployment update strategy for this service, overriding `defaults.strategy`. |
| services.render.tolerations | list | `[]` | Tolerations for this service's pods, overriding `defaults.tolerations`. |
| services.render.topologySpreadConstraints | list | `[]` | Topology spread constraints for this service's pods, overriding `defaults.topologySpreadConstraints`. |
| services.sync | object | `{"affinity":{},"config":{},"enabled":true,"image":{"repository":"timschoenle/tankovault-sync","tag":"v11.0.1@sha256:f6e79954546976ad40d877355731537626bd4fe3227a964b1a54c87b1b822e29"},"nodeSelector":{},"podAntiAffinity":"","podDisruptionBudget":{"enabled":false,"maxUnavailable":null,"minAvailable":1},"priorityClassName":"","replicaCount":1,"resources":{"limits":{"memory":"512Mi"},"requests":{"cpu":"100m","memory":"256Mi"}},"service":{"annotations":{},"type":"ClusterIP"},"strategy":{},"tolerations":[],"topologySpreadConstraints":[]}` | Bidirectional AniList integration and metadata enrichment. |
| services.sync.affinity | object | `{}` | Explicit affinity rules for this service, overriding `defaults.affinity`. Wins over `podAntiAffinity` when set. |
...
| services.sync.image | object | `{"repository":"timschoenle/tankovault-sync","tag":"v11.0.1@sha256:f6e79954546976ad40d877355731537626bd4fe3227a964b1a54c87b1b822e29"}` | Repository and tag for this service. Registry and pull policy come from the top-level `image`. |
| services.sync.image.repository | string | `"timschoenle/tankovault-sync"` | Image repository. |
| services.sync.image.tag | string | `"v11.0.1@sha256:f6e79954546976ad40d877355731537626bd4fe3227a964b1a54c87b1b822e29"` | Image tag, pinned by digest. |
| services.sync.nodeSelector | object | `{}` | Node selector for this service's pods, overriding `defaults.nodeSelector`. |
| services.sync.podAntiAffinity | string | `""` | Pod anti-affinity across nodes for this service, overriding `defaults.podAntiAffinity`. Empty defers to the default. `soft` prefers spreading replicas, `hard` requires it. |
| services.sync.podDisruptionBudget | object | `{"enabled":false,"maxUnavailable":null,"minAvailable":1}` | PodDisruptionBudget for this service's pods. |
...
| services.sync.priorityClassName | string | `""` | Priority class for this service's pods, overriding `defaults.priorityClassName`. |
...
| services.sync.resources | object | `{"limits":{"memory":"512Mi"},"requests":{"cpu":"100m","memory":"256Mi"}}` | Resource requests and limits. Sync reconciles against AniList in batches. |
| services.sync.resources.limits | object | `{"memory":"512Mi"}` | Resource limits. |
| services.sync.resources.limits.memory | string | `"512Mi"` | Maximum allowed memory usage. |
| services.sync.resources.requests | object | `{"cpu":"100m","memory":"256Mi"}` | Resource requests. |
| services.sync.resources.requests.cpu | string | `"100m"` | Minimum CPU requested. |
| services.sync.resources.requests.memory | string | `"256Mi"` | Minimum guaranteed memory allocation. |
| services.sync.service | object | `{"annotations":{},"type":"ClusterIP"}` | The Service in front of this deployment. Its port is fixed by the chart, so only the type and the annotations are open. |
...
| services.sync.strategy | object | `{}` | Deployment update strategy for this service, overriding `defaults.strategy`. |
| services.sync.tolerations | list | `[]` | Tolerations for this service's pods, overriding `defaults.tolerations`. |
| services.sync.topologySpreadConstraints | list | `[]` | Topology spread constraints for this service's pods, overriding `defaults.topologySpreadConstraints`. |
| services.worker | object | `{"affinity":{},"autoscaling":{"enabled":false,"maxReplicas":10,"minReplicas":2,"targetCPUUtilizationPercentage":80,"targetMemoryUtilizationPercentage":null},"config":{},"enabled":true,"image":{"repository":"timschoenle/tankovault-worker","tag":"v11.0.1@sha256:11e0ae2b72415625d5577e044a6e9147d2017bd2fedec5d731c2b454301037f9"},"nodeSelector":{},"podAntiAffinity":"","podDisruptionBudget":{"enabled":false,"maxUnavailable":null,"minAvailable":1},"priorityClassName":"","replicaCount":2,"resources":{"limits":{"memory":"1Gi"},"requests":{"cpu":"250m","memory":"512Mi"}},"service":{"annotations":{},"type":"ClusterIP"},"strategy":{},"tolerations":[],"topologySpreadConstraints":[]}` | Fetches and parses provider data through the adapters and upserts chapter and metadata changes. Scales horizontally for free: replicas join one NATS JetStream consumer group. |
| services.worker.affinity | object | `{}` | Explicit affinity rules for this service, overriding `defaults.affinity`. Wins over `podAntiAffinity` when set. |
| services.worker.autoscaling | object | `{"enabled":false,"maxReplicas":10,"minReplicas":2,"targetCPUUtilizationPercentage":80,"targetMemoryUtilizationPercentage":null}` | HorizontalPodAutoscaler for this service. While it is enabled the Deployment renders no `replicas`, so `replicaCount` is ignored. |
...
| services.worker.image | object | `{"repository":"timschoenle/tankovault-worker","tag":"v11.0.1@sha256:11e0ae2b72415625d5577e044a6e9147d2017bd2fedec5d731c2b454301037f9"}` | Repository and tag for this service. Registry and pull policy come from the top-level `image`. |
| services.worker.image.repository | string | `"timschoenle/tankovault-worker"` | Image repository. |
| services.worker.image.tag | string | `"v11.0.1@sha256:11e0ae2b72415625d5577e044a6e9147d2017bd2fedec5d731c2b454301037f9"` | Image tag, pinned by digest. |
| services.worker.nodeSelector | object | `{}` | Node selector for this service's pods, overriding `defaults.nodeSelector`. |
| services.worker.podAntiAffinity | string | `""` | Pod anti-affinity across nodes for this service, overriding `defaults.podAntiAffinity`. Empty defers to the default. `soft` prefers spreading replicas, `hard` requires it. |
| services.worker.podDisruptionBudget | object | `{"enabled":false,"maxUnavailable":null,"minAvailable":1}` | PodDisruptionBudget for this service's pods. |
...
| services.worker.priorityClassName | string | `""` | Priority class for this service's pods, overriding `defaults.priorityClassName`. |
| services.worker.replicaCount | int | `2` | Replica count, ignored when autoscaling is enabled. |
| services.worker.resources | object | `{"limits":{"memory":"1Gi"},"requests":{"cpu":"250m","memory":"512Mi"}}` | Resource requests and limits. The worker fetches and parses provider pages, which is the chart's most memory-hungry non-browser tier. |
| services.worker.resources.limits | object | `{"memory":"1Gi"}` | Resource limits. |
| services.worker.resources.limits.memory | string | `"1Gi"` | Maximum allowed memory usage. |
| services.worker.resources.requests | object | `{"cpu":"250m","memory":"512Mi"}` | Resource requests. |
| services.worker.resources.requests.cpu | string | `"250m"` | Minimum CPU requested. |
| services.worker.resources.requests.memory | string | `"512Mi"` | Minimum guaranteed memory allocation. |
| services.worker.service | object | `{"annotations":{},"type":"ClusterIP"}` | The Service in front of this deployment. Its port is fixed by the chart, so only the type and the annotations are open. |
...
| services.worker.strategy | object | `{}` | Deployment update strategy for this service, overriding `defaults.strategy`. |
| services.worker.tolerations | list | `[]` | Tolerations for this service's pods, overriding `defaults.tolerations`. |
| services.worker.topologySpreadConstraints | list | `[]` | Topology spread constraints for this service's pods, overriding `defaults.topologySpreadConstraints`. |
| statementTimeouts | object | `{"adminMaxConnections":5,"adminSecs":60,"interactiveSecs":30}` | The `statement_timeouts` settings. |
| statementTimeouts.adminMaxConnections | int | `5` | Connections the admin pool may hold, on top of `database.max_connections` (`statement_timeouts.admin_max_connections`). |
| statementTimeouts.adminSecs | int | `60` | Seconds one statement may run on a `/v1/admin` route or in the API's background sweeps. `0` removes the ceiling (`statement_timeouts.admin_secs`). |
| statementTimeouts.interactiveSecs | int | `30` | Seconds one statement may run on every non-admin route: `/v1/me`, the public catalogue and sign-in. `0` removes the ceiling (`statement_timeouts.interactive_secs`). |
...
| trawl.image | object | `{"repository":"ghcr.io/germondai/trawl","tag":"1.6.4@sha256:933d5fd2491b5f4f460e027f3f2cbde0ccbef96ea696dc7df67f9729005587d3"}` | Repository and tag for the bundled TRAWL. |
| trawl.image.repository | string | `"ghcr.io/germondai/trawl"` | Image repository. |
| trawl.image.tag | string | `"1.6.4@sha256:933d5fd2491b5f4f460e027f3f2cbde0ccbef96ea696dc7df67f9729005587d3"` | Image tag, pinned by digest. Never `latest`: it silently changes the bot-management behaviour the adapters are tuned against. |
...
| trawl.redis | object | `{"enabled":true,"url":""}` | TRAWL's own solved-session cache, which defaults to the Redis the services already use. |
...
| trawl.replicaCount | int | `1` | Replica count. TRAWL is stateless — its solved-session cache lives in `redis`, not in the pod — so this scales horizontally without a correctness concern. It does not scale memory for free, though: each replica keeps its own warm browser pool sized by `browserPoolSize`, so raising `replicaCount` multiplies memory cost by `browserPoolSize`, not just by the replica count. |
| trawl.resources | object | `{"limits":{"memory":"2Gi"},"requests":{"cpu":"500m","memory":"1Gi"}}` | Resource requests and limits. A browser tier needs far more headroom than the rest of the stack: each warm Firefox is hundreds of megabytes before it has solved anything. Scale with `browserPoolSize`. |
| trawl.resources.limits | object | `{"memory":"2Gi"}` | Ceiling for the pod. Memory only, as everywhere else in this chart. |
| trawl.resources.requests | object | `{"cpu":"500m","memory":"1Gi"}` | What the scheduler reserves for one TRAWL pod at `browserPoolSize: 1`. |
...
| valkey.resources | object | `{"limits":{"memory":"512Mi"},"requests":{"cpu":"100m","memory":"256Mi"}}` | Resource requests and limits for the cache container. The cache holds sessions and short-lived derived data, so its footprint is bounded by traffic rather than library size. |
| valkey.resources.limits | object | `{"memory":"512Mi"}` | Resource limits. |
| valkey.resources.limits.memory | string | `"512Mi"` | Maximum allowed memory usage. |
| valkey.resources.requests | object | `{"cpu":"100m","memory":"256Mi"}` | Resource requests. |
| valkey.resources.requests.cpu | string | `"100m"` | Minimum CPU requested. |
| valkey.resources.requests.memory | string | `"256Mi"` | Minimum guaranteed memory allocation. |
| valkey.tolerations | list | `[]` | Tolerations for Valkey's pod. |
| valkey.topologySpreadConstraints | list | `[]` | Topology spread constraints for Valkey's pod. |
