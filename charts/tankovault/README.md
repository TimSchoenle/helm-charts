...
| legal | object | `{"defaultLocale":null,"documents":null}` | The `legal` settings. |
...
| legal.documents | string | `nil` | The published documents, keyed by the slug their URL uses. A slug is lowercase letters, digits, `_` and `-`, at most 64 characters, and starts with a letter or a digit (`legal.documents`). |
