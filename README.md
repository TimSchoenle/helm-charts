...
Versions are deliberately omitted here: they change on nearly every pull request, and a
rendered version column turns each of those into a merge conflict on this file. Run
`helm search repo timschoenle --versions` for the published versions, or read the chart's
own README.
...
The [`common`](./charts/common) library chart is not published. It holds the
