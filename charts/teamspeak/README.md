...
![Version: 2.0.1](https://img.shields.io/badge/Version-2.0.1-informational?style=flat-square) ![AppVersion: 3.13.8](https://img.shields.io/badge/AppVersion-3.13.8-informational?style=flat-square)
...
No preset ever set a CPU limit, and this chart's default does not either — throttling a
real-time voice mixer produces audible stutter, and a CPU limit protects nobody but the node's
...
| database | object | `{"clientKeepDays":30,"connections":10,"existingSecretPasswordKey":"database-password","host":"","name":"teamspeak","password":"","plugin":"ts3db_sqlite3","port":3306,"user":"teamspeak","waitUntilReady":30}` | Where the server keeps its data. `ts3db_sqlite3` writes into the data volume; the other plugins need a `host` and a password. |
...
| networkPolicy.cilium | object | `{"description":"","egress":{"accountingFQDNs":[],"customRules":[],"dnsMatchPatterns":[],"entityPorts":[],"httpsFQDNs":[],"toEntities":[],"weblistFQDNs":[]},"enableDefaultDeny":true,"extraEgress":[],"extraIngress":[],"ingress":{"customRules":[],"fromEntities":[]}}` | Cilium-only additions, used when `engine` is `cilium` or `both`. Every rule above is translated automatically; these are the ones the portable API has no way to express.  `extraIngress`, `extraEgress` and the per-section `customRules` above are *not* carried over: those are verbatim `networking.k8s.io/v1` rule objects and are not valid CNP. The fields below are their counterparts. |
...
| persistence | object | `{"data":{"accessMode":"ReadWriteOnce","annotations":{},"enabled":true,"existingClaim":"","size":"3Gi","storageClassName":""}}` | The volumes the pod mounts. One today, holding everything under `/var/ts3server`. |
...
| resources | object | `{"limits":{"memory":"512Mi"},"requests":{"cpu":"100m","memory":"128Mi"}}` | Requests and limits for the server container, passed through to the pod spec unchanged. |
...
| server | object | `{"fileTransferPort":30033,"hintsEnabled":true,"logAppend":false,"logQueryCommands":false,"machineId":"","queryPort":10011,"querySshPort":10022,"voicePort":9987}` | The voice server's own ports and behaviour, written to the ConfigMap as `TS3SERVER_*` variables the image's entrypoint turns into `ts3server.ini`. |
...
| serverQuery | object | `{"adminPassword":"","existingSecretPasswordKey":"serveradmin-password","protocols":"raw","skipBruteForceCheck":false,"timeout":300}` | The `serveradmin` ServerQuery account and the interface it logs in over. `metrics.enabled` needs both a password it can be told and `raw` left in `protocols`. |
...
| service | object | `{"annotations":{},"externalIPs":[],"externalTrafficPolicy":"Local","fileTransfer":{"enabled":true,"nodePort":0},"ipFamilyPolicy":"","loadBalancerClass":"","loadBalancerIP":"","loadBalancerSourceRanges":[],"query":{"enabled":false,"nodePort":0},"querySsh":{"enabled":false,"nodePort":0},"type":"LoadBalancer","voice":{"enabled":true,"nodePort":0}}` | The Service that publishes the voice port. Clients dial `server.voicePort` directly, so the type has to be one that answers on that exact port. |
